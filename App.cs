...
namespace MQTT_SERVER
{
    public class App : BackgroundService
    {
        ...
        public App(ILogger<App> logger, AppSettings appSettings  )
        {
          ...
          .WithUrl(new Uri("http://10.4.4.224:11111/scalingHub"))
        ...
        private async Task RecevicedMyTopic(MqttApplicationMessageReceivedEventArgs e)
        {
            var payload = e.ApplicationMessage.Payload;
            string result = System.Text.Encoding.UTF8.GetString(payload);
            var MyTopic = JsonConvert.DeserializeObject<MyTopic>(result);
            Console.WriteLine($"Raw data = {Encoding.UTF8.GetString(e.ApplicationMessage.Payload)}");
            try
            {
                using (var context = new DBContext())
                {
                    var results = new RawDatas()
                    {
                        TimeRecieve = Convert.ToDateTime(MyTopic.Timestamp),
                        Count = MyTopic.boxCount.ToInt(),
                        Camera = MyTopic.Cam,
                        Line = MyTopic.Line
                    };
                    context.RawDatas.Add(results);
        ...
        private async Task<IMqttServer> CreateMqttServerAsync()
        {
            ...
            {
                MyConsole.Disconnected($"Client {e.ClientId} disconnected");
            });

            mqttServer.ClientConnectedHandler = new MqttServerClientConnectedHandlerDelegate(e =>
            {
                MyConsole.Connected($"Client {e.ClientId} connected");
            });
